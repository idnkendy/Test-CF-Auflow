...
const DrawingGenerator: React.FC<DrawingGeneratorProps> = ({ state, onStateChange, userCredits = 0, onDeductCredits, onInsufficientCredits }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            if (onDeductCredits) await onDeductCredits(cost, `Tạo Bản vẽ (${numberOfImages} ảnh) - ${resolution}`);
            ...
            if (result.imageUrls) {
                ...
                result.imageUrls.forEach(url => historyService.addToHistory({ tool: Tool.DrawingGenerator, prompt: fullPrompt, sourceImageURL: sourceImage.objectURL, resultImageURL: url }));
            }
        } catch (err: any) {
                    ...
                    <div className="bg-gray-100 dark:bg-black/20 p-4 rounded-2xl space-y-4 border border-gray-200 dark:border-white/5">
                        ...
                        <div>
                            ...
                            <div className="p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-white dark:bg-[#121212] shadow-inner">
                                <textarea rows={5} className="w-full bg-transparent outline-none text-sm resize-none font-medium text-text-primary dark:text-white" placeholder={language === 'vi' ? "VD: Mặt đứng chính, chi tiết cửa sổ, tỷ lệ chuẩn..." : "Ex: Main elevation, window details, standard scale..."} value={prompt} onChange={(e) => onStateChange({ prompt: e.target.value })} />
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="w-full bg-gray-100 dark:bg-[#121212] relative overflow-hidden flex flex-col items-start min-h-[400px]">
                        {resultImages.length > 0 ? (
                            ...
                            <div className="w-full h-full flex flex-col items-center justify-center py-40 opacity-20 select-none bg-main-bg dark:bg-[#121212]">
                                <span className="material-symbols-outlined text-6xl mb-4">blueprint</span>
