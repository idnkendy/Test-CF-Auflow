...
const ViewSync: React.FC<ViewSyncProps> = ({ state, onStateChange, userCredits = 0, onDeductCredits, onInsufficientCredits }) => {
    ...
    const creativeOptions = useMemo(() => [
        ...
        { 
            ...
            bg: 'https://images.unsplash.com/photo-1600607687920-4e2a09cf159d?q=80&w=800&auto=format&fit=crop'
        }
    ], [t]);
    ...
    const currentSlots = useMemo(() => creativeOption === 'architecture' ? architectureSlots : interiorSlots, [creativeOption, architectureSlots, interiorSlots]);
        ...
        if (creativeOption === 'architecture') {
            ...
            if (isVi) {
                fullPrompt = `Bạn là một Kiến trúc sư chuyên nghiệp. Bạn được cung cấp một hình ảnh mẫu kiến trúc đại diện cho 100% hình khối và chi tiết thực tế. Nhiệm vụ của bạn là vẽ lại một view kiến trúc cụ thể (cận cảnh hoặc nghệ thuật) từ công trình này.\nYÊU CẦU BẮT BUỘC:\n- GIỮ NGUYÊN 100% mọi chi tiết kiến trúc, hình khối, vật liệu và cấu trúc từ ảnh mẫu.\n- TUYỆT ĐỐI KHÔNG vẽ thêm, không sáng tạo chi tiết mới, không thay đổi cấu kiện nếu không có trong ảnh gốc. Chỉ được phép lấy đúng những gì đang có trên công trình gốc để thể hiện.\n- Đối với các view CẬN CẢNH và NGHỆ THUẬT: Bạn chỉ được phép tập trung vào những thành phần hiện hữu của công trình, không được phép nội suy hay sử dụng chi tiết lạ.\n- Bạn chỉ được phép thay đổi góc máy, tiêu cự ống kính, và điều kiện ánh sáng để tạo ra bức ảnh nhiếp ảnh kiến trúc chuyên nghiệp.\n- VIEW CẦN TẠO: "${viewDescription}".\n${charPrompt}\nKết quả phải là một ảnh chụp thực tế, 8k, sắc nét tuyệt đối.`;
                 ...
                 fullPrompt = `You are a professional Architect. You are provided with an architectural sample image representing 100% of the actual massing and details. Your task is to redraw a specific architectural view (close-up or artistic) from this building.\nMANDATORY REQUIREMENTS:\n- KEEP 100% of all architectural details, massing, materials, and structure from the sample image.\n- ABSOLUTELY DO NOT add, invent new details, or change components if they are not in the original image.\n- For CLOSE-UP and ARTISTIC views: Only focus on existing components.\n- VIEW TO GENERATE: "${viewDescription}".\n${charPrompt}\nPhotorealistic, 8k, absolutely sharp image.`;
            }
        } else if (creativeOption === 'interior-from-arch') {
            const viewName = slot.name;
            const charPrompt = characterImage 
                ? (isVi ? "Có thêm nhân vật đang sinh hoạt trong không gian." : "Include a character active in the space.")
                : "";
            if (isVi) {
                fullPrompt = `Bạn là một Kiến trúc sư và Nhà thiết kế nội thất tài ba. Bạn được cung cấp một hình ảnh NGOẠI THẤT. Nhiệm vụ của bạn là thiết kế không gian NỘI THẤT bên trong công trình đó.\nYÊU CẦU BẮT BUỘC:\n- PHONG CÁCH: Nội thất hoàn toàn đồng nhất với phong cách kiến trúc ngoại thất.\n- HỆ CỬA SỔ: Kiểu dáng khung cửa, vật liệu và tỷ lệ PHẢI giống hệt với hệ cửa sổ thấy được ở mặt tiền kiến trúc trong ảnh gốc.\n- VẬT LIỆU: Sử dụng bảng vật liệu và màu sắc tương đồng với ngoại thất.\n- KHÔNG GIAN CẦN TẠO: "${viewName}".\n${charPrompt}\nKết quả là một bức ảnh chụp nhiếp ảnh nội thất chuyên nghiệp, 8k.`;
            } else {
                 fullPrompt = `You are a talented Architect and Interior Designer. Design the INTERIOR space inside the building from the provided EXTERIOR image.\nREQUIREMENTS:\n- STYLE: Completely consistent with exterior style.\n- WINDOWS: Identical frame style and materials as the facade.\n- SPACE TO GENERATE: "${viewName}".\n${charPrompt}\nProfessional interior photograph, 8k.`;
            }
        } else {
            const action = slot.action || "active in the space";
            const charPrompt = characterImage 
                ? (isVi 
                    ? `THÊM NHÂN VẬT: Hãy đưa nhân vật trong ảnh thứ hai vào không gian một cách tự nhiên. Nhân vật nên ${action}. Đảm bảo trang phục và ngoại hình thống nhất với ảnh nhân vật được cung cấp.` 
                    : `ADD CHARACTER: Naturally integrate the character from the second image. The character should be ${action}. Consistent appearance with the provided character image.`)
                : "";
            if (isVi) {
                fullPrompt = `Bạn là một Kiến trúc sư nội thất chuyên nghiệp. Bạn được cung cấp một hình ảnh mẫu đại diện cho style, màu sắc và vật liệu. Nhiệm vụ của bạn là tưởng tượng và vẽ ra một không gian khác trong cùng ngôi nhà đó.\nYÊU CẦU BẮT BUỘC:\n- GIỮ NGUYÊN Style thiết kế, Bảng màu chủ đạo và Tính chất vật liệu.\n- KHÔNG GIAN CẦN TẠO: "${slot.name}".\n${charPrompt}\nHãy vẽ một bức ảnh chụp nhiếp ảnh kiến trúc chuyên nghiệp, thực tế, 8k.`;
            } else {
                fullPrompt = `You are a professional Interior Architect. Imagine and draw another space in the same house as the provided sample image.\nREQUIREMENTS:\n- KEEP design style, dominant color palette, and material properties.\n- SPACE TO GENERATE: "${slot.name}".\n${charPrompt}\nProfessional architectural photograph, realistic, 8k.`;
            }
        }
        return fullPrompt;
    };
    ...
    const handleGenerateBatch = async () => {
        ...
        onStateChange({ error: null });
        try {
            if (onDeductCredits) await onDeductCredits(creativeTotalCost, `Creative Batch: ${currentSlots.length} views (${resolution})`);
            const modelName = resolution === 'Standard' ? "GEM_PIX" : "GEM_PIX_2";
            const inputImages = [sourceImage];
            if (characterImage) inputImages.push(characterImage);
            const promises = currentSlots.map(async (slot) => {
                ...
                } catch (err) { console.error(`Error generating view ${slot.name}:`, err); } finally { setGeneratingViews(prev => { const next = new Set(prev); next.delete(uniqueKey); return next; }); }
            });
