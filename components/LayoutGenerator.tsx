...
const LayoutGenerator: React.FC<LayoutGeneratorProps> = ({ state, onStateChange, userCredits = 0, onDeductCredits }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const { data: { user } } = await supabase.auth.getUser();
            if (user && logId && onDeductCredits) await refundCredits(user.id, cost, `Hoàn tiền: Lỗi layout (${rawMsg})`, logId);
        } finally {
    ...
    const handleDownload = async () => {
        ...
        await externalVideoService.forceDownload(resultImages[0], `layout-generated-${Date.now()}.png`);
