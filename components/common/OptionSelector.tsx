    ...
    <div className={`relative ${isOpen ? 'z-[50]' : 'z-10'}`} ref={containerRef}>
      ...
      {isOpen && (
        ...
        >
          ...
          <div className="max-h-72 overflow-y-auto p-2 scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600">
            {variant === 'grid' ? (
                <div className="grid grid-cols-2 gap-2">
                    {options.map((option) => (
                        <button
                            ...
                            className={`
                            relative flex items-center justify-center px-2 py-3 rounded-xl text-sm font-medium transition-all duration-200 border
                            ${value === option.value 
                ...
                <div className="flex flex-col gap-1">
                    ...
                    >
                        <span className="truncate">{option.label}</span>
                        {value === option.value && (
                        <span className="w-5 h-5 flex items-center justify-center bg-primary text-white rounded-full text-[10px]">
