...
const RealEstatePoster: React.FC<RealEstatePosterProps> = ({ state, onStateChange, userCredits = 0, onDeductCredits, onInsufficientCredits }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            if (onDeductCredits) await onDeductCredits(cost, `Tạo Poster BDS (${numberOfImages} ảnh) - ${resolution}`);
            ...
            if (result.imageUrls) {
                ...
                result.imageUrls.forEach(url => historyService.addToHistory({ tool: Tool.RealEstatePoster, prompt: fullPrompt, sourceImageURL: sourceImage.objectURL, resultImageURL: url }));
            }
        } catch (err: any) {
