...
const RealEstatePoster: React.FC<RealEstatePosterProps> = ({ state, onStateChange, userCredits = 0, onDeductCredits, onInsufficientCredits }) => {
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            const { data: { user } } = await supabase.auth.getUser();
            if (user && logId && onDeductCredits) await refundCredits(user.id, cost, `Hoàn tiền: Lỗi poster (${rawMsg})`, logId);
        } finally {
    ...
    const handleDownload = async () => {
        ...
        await externalVideoService.forceDownload(resultImages[0], `real-estate-poster-${Date.now()}.png`);
