                ...
                <div className="p-3 space-y-4 flex-1 lg:overflow-y-auto custom-sidebar-scroll overflow-visible">
                    ...
                    <div className="bg-gray-100 dark:bg-black/20 p-3 sm:p-4 rounded-2xl space-y-4 border border-gray-200 dark:border-white/5">
                        ...
                        <div className="space-y-3">
                            ...
                            {renderMode === 'urban' && (
                                <><OptionSelector id="v-type" label={t('ext.urban.view_type')} options={viewTypeOptions} value={viewType} onChange={(v) => syncPrompt({ viewType: v })} variant="select" /><div className="grid grid-cols-2 gap-2"><OptionSelector id="v-density" label={t('ext.urban.density')} options={densityOptions} value={density} onChange={(v) => syncPrompt({ density: v })} variant="select" /><OptionSelector id="v-light" label={t('opt.lighting')} options={lightingOptions} value={lighting} onChange={(v) => syncPrompt({ lighting: v })} variant="select" /></div></>
                            )}
                            {renderMode === 'landscape' && (
                                <><OptionSelector id="g-style" label={t('ext.landscape.style')} options={gardenStyleOptions} value={gardenStyle} onChange={(v) => syncPrompt({ gardenStyle: v })} variant="select" /><div className="grid grid-cols-2 gap-2"><OptionSelector id="g-feature" label={t('ext.landscape.feature')} options={featureOptions} value={features} onChange={(v) => syncPrompt({ features: v })} variant="select" /><OptionSelector id="g-time" label={t('ext.landscape.time')} options={lightingOptions} value={lighting} onChange={(v) => syncPrompt({ lighting: v })} variant="select" /></div></>
                            )}
                        </div>
                    </div>
                        ...
                        {resultImages.length > 0 ? (
                            <div className="w-full h-full p-2 animate-fade-in flex flex-col items-center justify-center relative">
                                ...
                                <div className="absolute top-4 right-4 flex flex-col gap-2 z-10">
                                    <button onClick={() => onSendToViewSync(sourceImage!)} className="p-1.5 sm:p-2 bg-white/90 dark:bg-black/50 rounded-xl shadow-lg hover:text-purple-600 transition-all backdrop-blur-sm border border-white/20"><span className="material-symbols-outlined text-base sm:text-lg notranslate">view_in_ar</span></button>
                                    <button onClick={handleDownload} className="p-1.5 sm:p-2 bg-white/90 dark:bg-black/50 rounded-xl shadow-lg hover:text-blue-600 transition-all backdrop-blur-sm border border-white/20"><span className="material-symbols-outlined text-base sm:text-lg notranslate">download</span></button>
                                    <button onClick={() => setPreviewImage(resultImages[selectedIndex])} className="p-1.5 sm:p-2 bg-white/90 dark:bg-black/50 rounded-xl shadow-lg hover:text-green-600 transition-all backdrop-blur-sm border border-white/20"><span className="material-symbols-outlined text-base sm:text-lg notranslate">zoom_in</span></button>
                                </div>
                            </div>
                        ) : (
